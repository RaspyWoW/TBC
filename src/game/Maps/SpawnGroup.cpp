...
void SpawnGroup::Spawn(bool force)
{
    ...
    for (auto itr = eligibleGuids.begin(); itr != eligibleGuids.end() && !eligibleGuids.empty() && m_objects.size() < m_entry.MaxCount; ++itr)
    {
        ...
        if (GetObjectTypeId() == TYPEID_UNIT)
        {
            const auto data = sObjectMgr.GetCreatureData(dbGuid);
            x = data->posX; y = data->posY;
            m_map.GetPersistentState()->AddCreatureToGrid(dbGuid, data);
        }
        else
        {
            const auto data = sObjectMgr.GetGOData(dbGuid);
            x = data->posX; y = data->posY;
...
{
    const float defaultDist = m_currentSpread;
    auto& slots = m_slotsMap;
    float totalMembers = 0;
    const bool onlyAlive = HaveOption(SPAWN_GROUP_FORMATION_OPTION_KEEP_CONPACT);
    ...
    switch (m_currentFormationShape)
    {
        ...
        case SPAWN_GROUP_FORMATION_TYPE_RANDOM:
        {
            ...
            for (auto& slotItr : m_slotsMap)
            {
                ...
                const float testInterSpread = modelWidth * 3.0f;
                float finalAngle = 0;
                float finalDist = 0;
                switch (membCount)
                {
                    case 1:
                    {
                        const float distBack = m_currentSpread + testInterSpread;
                        const float hyp = std::sqrt(distBack * distBack + testInterSpread * testInterSpread);
                    ...
                    case 5:
                    {
                        const float distBack = m_currentSpread + (testInterSpread * 2.0f);
                        const float hyp = std::sqrt(distBack * distBack + testInterSpread * testInterSpread);
                    ...
                    case 7:
                    {
                        const float distBack = m_currentSpread + (testInterSpread * 3.0f);
                        const float hyp = std::sqrt(distBack * distBack + testInterSpread * testInterSpread);
                    ...
                    default:
                    {
                        ...
                        finalDist = testInterSpread * (static_cast<float>((membCount - 10) / 2) + 1);
                        break;
                    }
                }
