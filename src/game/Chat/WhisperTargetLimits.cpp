...
void WhisperTargetLimits::SaveTargets() const
{
    CharacterDatabase.PExecute("DELETE FROM `whisper_targets` WHERE `account` = %u", account_id_);

    for (const auto& target : targets_)
    {
        CharacterDatabase.PExecute("REPLACE `whisper_targets` (`account`, `target_guid`, `time`) VALUES (%u, %u, %lu)", account_id_, target.first, target.second);
    }
}

void WhisperTargetLimits::LoadTargets()
{
...
bool WhisperTargetLimits::CanWhisper(const std::uint32_t target_guid, const std::uint32_t level)
{
...
void WhisperTargetLimits::TargetDecay(const std::time_t& time)
{
