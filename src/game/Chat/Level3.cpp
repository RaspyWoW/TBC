...
bool ChatHandler::HandleDieHelper(Unit* target)
{
    ...
    if (target->IsAlive())
    {
        if (sWorld.getConfig(CONFIG_BOOL_DIE_COMMAND_CREDIT))
        {
            ...
            m_session->GetPlayer()->DealDamageMods(player, target, damage, 0, DIRECT_DAMAGE);
            m_session->GetPlayer()->DealDamage(player, target, damage, nullptr, DIRECT_DAMAGE, SPELL_SCHOOL_MASK_NORMAL, nullptr, false);
        }
        else
        {
            ...
            uint32 damage = target->GetHealth();
            target->DealDamageMods(target, target, damage, 0, DIRECT_DAMAGE);
            target->DealDamage(target, target, damage, nullptr, DIRECT_DAMAGE, SPELL_SCHOOL_MASK_NORMAL, nullptr, false);
        }
    }
...
bool ChatHandler::HandleBanInfoHelper(uint32 accountid, char const* accountname)
{
    ...
    do
    {
        ...
        std::string manuallyUnbanned{ GetMangosString(LANG_BANINFO_YES) };

        PSendSysMessage(LANG_BANINFO_HISTORYENTRY,
            fields[0].GetString(), bantime.c_str(), active ? GetMangosString(LANG_BANINFO_YES) : GetMangosString(LANG_BANINFO_NO), "", "");
    }
    while (result->NextRow());
...
bool ChatHandler::HandleBanInfoIPCommand(char* args)
{
    ...
    QueryResult const* result = LoginDatabase.PQuery("SELECT `ip`, FROM_UNIXTIME(`bandate`), FROM_UNIXTIME(`unbandate`), `unbandate`-UNIX_TIMESTAMP(), `banreason`,`bannedby`,`unbandate`-`bandate` FROM `ip_banned` WHERE `ip` = '%s'", IP.c_str());
    if (!result)
    {
...
bool ChatHandler::HandleBanListHelper(QueryResult* result)
{
    ...
    if (m_session)
    {
        do
        {
            ...
            QueryResult const* banresult = LoginDatabase.PQuery("SELECT `account`.`username` FROM `account`,`account_banned` WHERE `account_banned`.`id`='%u' AND `account_banned`.`id`=`account`.`id`", accountid);
            if (banresult)
            {
