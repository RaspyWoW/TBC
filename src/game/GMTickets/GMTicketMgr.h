...
{
    public:
        explicit GMTicket(Player const* player, const std::string message, const uint8 category, const time_t when = time(nullptr));
        explicit GMTicket(Field const* fields) { Load(fields); }

        static bool Compare(GMTicket const* a, GMTicket const* b);

        void Load(Field const* fields);
        inline void Save(SqlStatement& stmt) const;
        void Recycle(Player const* player, const std::string message, const uint8 category, const time_t when = time(nullptr));

        inline uint32                   GetId()             const { return m_id; }
        inline static std::string       GetIdTag(const uint32 id) { return ("T#" + std::to_string(id)); }
        ...
        inline bool IsAssignedTo(ObjectGuid const guid)     const { return (guid == m_assigneeGuid); }
        ...
        void SetState(GMTicketState const state, const time_t when = time(nullptr));
        void SetCategory(const uint8 id) { m_category = id; }

        void SetAnswered(const time_t when) { m_answered = when; }
        void SetSeen(const time_t when) { m_statsLastSeen = m_seen; m_seen = when; }
        void SetUpdated(const std::string &text, const time_t when = time(nullptr));

        void SetAssignee(const uint8 level, WorldSession const* session = nullptr);
        void SetConclusion(const std::string& response) { m_conclusion = response; }

        void UpdateStatsChat(const bool incoming);
...
{
    public:
        explicit GMSurveyResult(const GMSurveyEntry &entry, const uint32 ticketId, const std::map<uint32, uint8> &answers, const std::string &feedback);
...
class GMTicketMgr
{
    ...
    public:
        ...
        static void Save(GMTicket const* ticket);
        static void Save(GMSurveyResult const* survey);

        static void Print(GMTicket const& ticket, WorldSession* session, const time_t now = time(nullptr));
        static inline const std::string PrintMailResponse(GMTicket const& ticket, const bool resolved);
        static inline const char* PrintTicketCategory(GMTicket const& ticket, LocaleConstant const locale = DEFAULT_LOCALE);
        static inline const char* PrintTicketChatAlert(GMTicketMgrChatAlert const alert, LocaleConstant const locale = DEFAULT_LOCALE);
        static inline const char* PrintTicketStatus(GMTicket const& ticket, LocaleConstant const locale = DEFAULT_LOCALE);
        static inline const char* PrintTicketStatusColorSequence(GMTicket const& ticket);
        static inline const std::string PrintTicketSummaryLine(GMTicket const& ticket, LocaleConstant const locale = DEFAULT_LOCALE);

        void PrintTicketList(WorldSession const* session, std::ostringstream& output, const size_t max, GMTicketCategoryEntry const* category = nullptr, const bool online = false) const;

        static bool TicketChatIncoming(GMTicket* ticket, Player const* player, WorldSession const* gm, const std::string& message, const bool addon = false);
        static bool TicketChatOutgoing(GMTicket* ticket, WorldSession const* gm, Player const* player, const std::string& message, const bool addon = false);
        static bool TicketChatAlert(GMTicket const* ticket, GMTicketMgrChatAlert const alert);

        void OnPlayerOnlineState(Player &player, bool online) const;
        void ShowMOTD(Player &gm) const;

        std::pair<bool, bool> HookGMTicketWhisper(Player* sender, const std::string& recepient, Player const* character, const std::string &msg, const bool addon);
        bool HookGMTicketWhoQuery(const std::string& query, Player const* gm) const;

        GMTicket* GetTicketById(const uint32 id, GMTicketState const state = GMTICKET_STATE_OPEN) const;
        GMTicket* GetTicketByPlayer(ObjectGuid const playerGuid, GMTicketState const state = GMTICKET_STATE_OPEN, ObjectGuid const assigneeGuid = ObjectGuid()) const;

        bool Add(GMTicket* ticket, const bool loading = false);
        bool Recycle(GMTicket* ticket);

        void Survey(GMTicket* ticket, GMSurveyResult const* result = nullptr);

        CommandResult Abandon(GMTicket* ticket);
        inline CommandResult Discard(GMTicket* ticket, const std::string& reason, WorldSession const* session = nullptr) { return Close(ticket, reason, false, session); }
        CommandResult Escalate(GMTicket* ticket, WorldSession const* session, uint8 level);
        CommandResult Comment(GMTicket* ticket, WorldSession const* session, const std::string& text);
        CommandResult Read(GMTicket* ticket);
        inline CommandResult Resolve(GMTicket* ticket, const std::string& commentary, WorldSession const* session = nullptr) { return Close(ticket, commentary, true, session); }
        CommandResult Sort(GMTicket* ticket, const GMTicketCategoryEntry& category, WorldSession const* session = nullptr);
        CommandResult Update(GMTicket* ticket, const std::string& text);
        CommandResult Whisper(GMTicket* ticket, const std::string& message, WorldSession const* session = nullptr);

        static inline float GetDaysPassed(const time_t now, const time_t since) { return (now && since ? std::stof(std::to_string((difftime(now, since) / DAY))) : 0); }

        static inline float GetTicketAgeDays(const time_t now, const time_t created) { return GetDaysPassed(now, created); }
        inline float GetOldestTicketAgeDays() const { return m_statsOldestTicketAgeDays; }
        float GetOldestTicketAgeLastUpdateDays(const time_t now) const;

        inline bool GetSystemStatus() const { return m_status; }
        bool SetSystemStatus(const bool status);

        inline uint32 GenerateTicketId() { return ++m_lastTicketId; }

    private:
        static inline CommandResult CanComment(GMTicket const* ticket, WorldSession const* who);
        static inline CommandResult CanClose(GMTicket const* ticket, const bool resolve, WorldSession const* who);
        static inline CommandResult CanSort(GMTicket const* ticket, WorldSession const* who);
        static inline CommandResult CanWhisper(GMTicket const* ticket, WorldSession const* who);

        std::pair<GMTicket*, std::string> GetTicketByIdTag(const std::string& tag, GMTicketState const state = GMTICKET_STATE_OPEN, ObjectGuid const assigneeGuid = ObjectGuid()) const;

        bool Remove(GMTicket& ticket);
        void RemoveAll();

        CommandResult Close(GMTicket* ticket, const std::string& commentary, const bool resolved, WorldSession const* session = nullptr);

        void UpdateTicketQueueStats(GMTicket& closed, const time_t when);
        inline void UpdateTicketQueueTimers(GMTicket const* except = nullptr) const;
